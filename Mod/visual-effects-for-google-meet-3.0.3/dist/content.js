...
if (location.pathname.match(/\-/g).length >= 2) {
  ...
  class OpticElement {
    constructor(type, options) {
      let defTypes = ["text", "line", "empty", "checkbox"];
      let defOptions = {
        text: "",
        parent: "",
        label: "",
    ...
    createElem() {
      switch (this.type) {
        case "text":
          ...
          let className =
            "optic-checked" + this.options.bold
              ? " bold"
              : "" + this.options.class
              ? " " + this.options.class
              : "";
          ...
          textElement.appendChild(document.createTextNode(this.options.text));
  ...
  class GoogleMeetOptics {
    ...
    async createUIElements() {
      function insertHTML(html, element = document.body) {
        var htmlCreated = document.createElement("div");
        htmlCreated.className = "inserted";
        element.appendChild(htmlCreated);
        document.querySelector(".inserted").outerHTML = html;
      }
      while (document.querySelector(".d7iDfe") !== null) {
        await new Promise((r) => setTimeout(r, 500));
      }
      while (document.querySelectorAll(".optic-menu").length >= 1) {
        document.querySelectorAll(".optic-menu")[0].outerHTML = "";
      }
      insertHTML(`<div class="optic-menu"> <br /></div>`);
      // Sometimes 2?
      ...
      new OpticElement("line", {});
      new OpticElement("text", { text: "Soon More!" });
